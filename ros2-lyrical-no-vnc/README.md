# ROS 2 Lyrical + noVNC + LXQt

该镜像使用 LXQt 2 桌面、TurboVNC 图形后端和 noVNC 1.7 Web 客户端。浏览器直接访问 `6080` 端口，无需安装 VNC Viewer，也无需挂载宿主机 `DISPLAY` / `.Xauthority` 或启用 `--privileged`。
...
```shell
docker build -t ros2-lyrical-novnc ./ros2-lyrica-no-vnc
```
...
```shell
docker exec -it ros2-lyrical-novnc zsh
rviz2
```
